...
int main(int argc, char** argv) {
    ...
    SwitchArg erosionFlag("E", "erosion", "enable erosion (disabled)", false);
    SwitchArg dilationFlag("D", "dilation", "enable dilation (disabled)", false);
    ...
    if (verboseSwitch.isSet()) {
        ...
        cout << left << setw(MAXWIDTH) << setfill(filler) << "Erosion" << ": " << erosionFlag.isSet() << endl;
        cout << left << setw(MAXWIDTH) << setfill(filler) << "Dilation" << ": " << dilationFlag.isSet() << endl;
    ...
    while (frameidx >= 0 and frameidx < dcnt) {
        ...
        //cout << Subtractor.learningStage << endl;
        Subtractor.getBackgroundImage(bgmodel); // get Background Model
        
        if (erosionFlag.isSet()) { erode(tmpMask,tmpMask,cv::Mat()); }
        if (dilationFlag.isSet()){ dilate(tmpMask,tmpMask,cv::Mat());}
        ...
        //dilate(bgmodel,bgmodel,cv::Mat());
        cvtColor(dframe,frameYCrCb,CV_BGR2YCrCb);
